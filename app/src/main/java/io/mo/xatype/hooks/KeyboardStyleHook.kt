...
object KeyboardStyleHook {
    ...
    private val compositorGlass = CompositorGlassSurface("h")
            ...
            if (oMethod != null) {
                module.hook(oMethod).intercept { chain ->
                    ...
                    val tracksTransparentGeometry = shouldKeepGlassNavigationTransparent() &&
                        ConfigManager.getOpacity() == 0 &&
                        compositorGlass.owns(XposedUtils.getObjectField(chain.thisObject, "h"))
    ...
    ): Boolean = compositorGlass.ensure(module, service, material, syncWithDraw)

    private fun removeDynamicGlassSurfacePrimer() = compositorGlass.remove()
